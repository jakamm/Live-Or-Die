...
public class FootstepsController : MonoBehaviour
{
    ...
    void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        if (playerVelocity > playVelocity)
        {
            ...
            {
                if (!isInWater && !isOnLadder && playerController.IsGrounded())
                {
    ...
    AudioClip[] GetSurfaceFootsteps()
    {
        if (surfaceDetails)
        {
            if (terrainUnder != null)
            {
                ...
                if (surface != null && surface.SurfaceFootsteps.Length > 0 && surface.allowFootsteps)
                {
                    return surface.SurfaceFootsteps;
                }
            }
