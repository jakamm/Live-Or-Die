...
public class DragRigidbody : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            RotateButton = crossPlatformInput.GetInput<bool>("Fire");

            if (objectRaycast && !antiSpam && !gameManager.isWeaponZooming)
            {
                if (crossPlatformInput.GetActionPressedOnce(this, "Examine"))
                {
                    GrabObject = !GrabObject;
                }
            }

            if (crossPlatformInput.GetActionPressedOnce(this, "Zoom") && objectHeld)
            {
                ThrowObject();
            }
            ...
            {
                Vector2 delta = crossPlatformInput.GetMouseDelta();
                zoomInputY = crossPlatformInput.GetMouseScroll().y * objectZoomSpeed;
