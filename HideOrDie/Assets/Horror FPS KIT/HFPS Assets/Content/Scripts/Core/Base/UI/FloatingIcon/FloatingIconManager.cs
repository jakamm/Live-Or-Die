        ...
        if (FloatingIconCache.Count > 0 && IsVisibleGlobal)
        {
            for (int i = 0; i < FloatingIconCache.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        Pair.Icon.SetIconVisible(interactiveItem.floatingIconEnabled);
                    }
        ...
        if (FloatingIconCache.Count > 0)
        {
            for (int i = 0; i < FloatingIconCache.Count; i++)
            {
                ...
                if ((pair = FloatingIconCache[i]) != null)
                {
                    ...
                    {
                        if ((interactiveItem = pair.FollowObject.GetComponent<InteractiveItem>()) != null && interactiveItem.floatingIconEnabled)
                        {
