...
{  
    ...
    void Update()
    {
        ...
        if (Inventory.CheckItemInventory(ItemInventoryID) && scriptManager.ScriptGlobalState && isSelected && !wallHit && !isBlocked && !isHideAnim)
        {
    ...
    void SpawnHitmark(SurfaceDetails surface, RaycastHit hit)
    {
        ...
        if (canSpawn && surface.SurfaceMeleemark && surface.allowImpactMark)
        {
            GameObject mark = Instantiate(surface.SurfaceMeleemark, hit.point, Quaternion.FromToRotation(Vector3.up, hit.normal));
            ...
            if (surface.MeleemarkImpact.Length > 0 && mark.GetComponent<AudioSource>())
            {
                AudioSource markAS = mark.GetComponent<AudioSource>();
                markAS.clip = surface.MeleemarkImpact[Random.Range(0, surface.MeleemarkImpact.Length)];
                markAS.volume = surface.ImpactVolume;
                markAS.Play();
            }
        }
        else if(surface.MeleemarkImpact.Length > 0)
        {
            AudioClip clip = surface.MeleemarkImpact[Random.Range(0, surface.MeleemarkImpact.Length)];
            AudioSource.PlayClipAtPoint(clip, hit.transform.position, surface.ImpactVolume);
        }
    }
