...
public class CandleItem : SwitcherBehaviour, ISaveableArmsItem, IItemValueProvider
{
    ...
    public void BlowOut_Event()
    {
        ...
        if (blowOutKeepCandle && Candle.transform.localScale.y > scaleKeepCandle)
        {
            ...
            {
                inventory.AddItem(InventoryID, 1, new CustomItemData(new Dictionary<string, string>() { { Inventory.ITEM_VALUE, OnGetValue() } }));
            }
        }
    }
    ...
    void Update()
    {
        ...
        {
            if (crossPlatformInput.ActionExist("HideItem"))
            {
                BlowOutKey = crossPlatformInput.GetInput<bool>("HideItem");
            }
        }

        CandleFlame.transform.position = FlamePosition.position;

        if (BlowOutKey && !IsPressed && isSelected && !anim.isPlaying && !IsBlocked)
        {
            anim.Play(BlowOutAnimation);
            StopAllCoroutines();
            IsPressed = true;
