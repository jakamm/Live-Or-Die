...
public class BreakableObject : MonoBehaviour, ISaveable
{
    ...
    void BreakObject()
    {
        ...
        if (SpawnObjectInside && ObjectsInside.Length > 0)
        {
            ...
            if (RandomObject)
            {
                int random = Random.Range(0, ObjectsInside.Length);

                insideObj = saveGameHandler.InstantiateSaveable(ObjectsInside[random], Center.position, SpawnedRotation);
            }
            else
            {
                insideObj = saveGameHandler.InstantiateSaveable(ObjectsInside[0], Center.position, SpawnedRotation);
            }

            if (AddFloatingIcon)
            {
                FloatingIconManager.Instance.FloatingIcons.Add(insideObj);
            }
        }
